...
const sendEmailSchema = z
  .object({
    ...
    subject: z.string().trim().min(1, "Subject is required.").max(998, "Subject is too long."),
...
const serverError = () =>
  NextResponse.json({ message: "Unable to send email right now." }, { status: 500 })
...
export const POST = async (request: Request) => {
  ...
  try {
    ...
    const { error: eventError } = await supabaseAdmin.from("lead_events").insert({
      lead_id: lead.id,
      actor_id: user.id,
